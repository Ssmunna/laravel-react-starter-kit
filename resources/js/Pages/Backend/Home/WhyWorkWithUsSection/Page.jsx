...
export default function Page({data:content}){
    ...
    const removeImage = (oldId, id) => {
        setData('images',  data.images.filter((image, index) => image.id !== id))
    }


    const handleSubmitForm = (e) => {
        e.preventDefault();
        post(route('admin.home.work_with_us.update'), {
            preserveState: true,
            preserveScroll: true,
        })
    }
                                    ...
                                    <div key={image.id} className={`w-[130px] h-[150px] relative shrink-0`}>
                                        ...
                                        { i === data.images.length - 1 && (
                                            <button type='button'
                                                    ...
                                                    className={`h-[30px] w-[30px] flex items-center justify-center bg-blue-500 text-white absolute bottom-[-40px] right-[0px] md:-bottom-[7px] md:right-[-33px] text-[25px] cursor-pointer`}
                                            >
                                                +
                                            </button>
                                        )}
